...
T62|.|DAG-driven drive loop — the active orchestrator. Each step: plan ready/next picks the next ready node -> dispatch a SCOPED prompt (only that node's spec slice + acceptance) -> collect TurnResult -> verify -> mark done/fail -> advance. Done = all nodes green (deterministic), not 'model said done'. Bounded by [driver] max_iters.|T60|V34,V27   (THE answer to 'we wait for the smart model': DAG drives, model fills leaves. Architecture (from ironclaw, the mature Rust reference): ONE canonical executor + a strategy-composed planner — context/compaction/model-pick/stop/budget/recovery/gate as swappable traits. Our T63(verify), T64(gate/tripwire), T66(input-drain/queues), T67(recovery/doomloop), T70(budget) ARE those strategies behind this executor + the T60 host-port. Moat vs pi/zerostack/caveman: an authoritative DAG none of them has.)
