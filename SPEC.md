...
T61|.|ApiDriver via the `rig` crate — multi-provider API backend (OpenRouter/Anthropic/local). Primary backend for the standalone harness; API-direct made affordable by mixing/routing providers.|T60|§I   (rig ~0.37 chosen (mature ecosystem — awesome-rig confirms lots built on it). Use rig-core DIRECTLY behind the Driver trait (our trait IS the dyn-dispatch seam — skip rig-dyn @1★). taquba-research + coral-rs prove rig-behind-a-trait. Optional local/cheap tier: rig-llama-cpp (GGUF). Wire rig-tap early for drive-loop observability. provider+model from [driver] config. Routing/repair/cache = SEPARATE future server (CCR+failover embryo), not here.)
T62|.|DAG-driven drive loop — the active orchestrator. Each step: plan ready/next picks the next ready node -> dispatch a SCOPED prompt (only that node's spec slice + acceptance) -> collect TurnResult -> verify -> mark done/fail -> advance. Done = all nodes green (deterministic), not 'model said done'. Bounded by [driver] max_iters.|T60|V34,V27   (THE answer to 'we wait for the smart model': DAG drives, model fills leaves. Architecture (from ironclaw): ONE canonical executor + strategy-composed planner — context/compaction/model-pick/stop/budget/recovery/gate as swappable traits; T63(verify),T64(gate),T66(input-drain),T67(recovery),T70(budget) ARE those strategies behind this executor + the T60 host-port. BUILD the topo ready-set scheduler ourselves; STEAL weavegraph's concurrency idioms (buffer_unordered frontier, barrier-merge, versioned state) + graph-flow's FanOutTask — do NOT adopt weavegraph as engine (its BSP/version-gated grain fights our authoritative ready-set DAG; harness must keep control). Moat: an authoritative DAG none of pi/zerostack/caveman has.)
...
T70|.|Budget as a loop primitive — track token/$ spend per loop and per node; a first-class termination condition and a cheap/expensive model-routing signal. WRAP squeez token-track, do not reimplement.|T62|V34   (Stolen from caveman-code (money as control signal). Wrap squeez token-track, never reimplement. Limits are ROUGH, not rigorous (user): approximate token/iteration/page caps whose job is twofold — (1) bound runaway work (the infinite-research spiral observed on bare Sonnet), (2) let the planner SELECT which elements fit within the envelope. A rough limit that SHAPES selection beats a precise limit that only kills. Pairs with T64 tripwire + the Explorer-Kitty research leash.)
